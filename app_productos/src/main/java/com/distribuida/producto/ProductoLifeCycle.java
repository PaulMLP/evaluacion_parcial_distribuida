...
@ApplicationScoped
public class ProductoLifeCycle {
    ...
    public void init(@Observes StartupEvent evt, Vertx vertx) throws UnknownHostException {
        System.out.println("***ProductoLifeCycle Init***");
        ...
        client.registerServiceAndAwait(
                new ServiceOptions()
                        .setName("app-productos")
                        .setId(serviceId)
                        .setAddress(ipAddress.getHostAddress())
                        .setPort(port)
                        .setTags(
                                List.of("traefik.enable=true",
                                        "traefik.http.routers.app-productos.rule=PathPrefix(`/app-productos`)",
                                        "traefik.http.routers.app-productos.middlewares=app-productos",
                                        "traefik.http.middlewares.app-productos.stripPrefix.prefixes=/app-productos"
                                )
                        )
                        .setCheckOptions(
                                new CheckOptions()
    ...
    public void stop(@Observes ShutdownEvent stevt, Vertx vertx) {
        System.out.println("***ProductoLifeCycle Stop***");
